...
Newsletter-api integrerer med eksterne e-postleverandører (f.eks. SendGrid) og krever gyldige API-nøkler.

### Oppgave 5.1: Overvåk API-nøkler for e-post
- Sørg for at API-nøkler for eksterne tjenester (SendGrid etc.) er lagret i Key Vault.
- Aktiver varsling for utløp av disse nøklene dersom tjenesten støtter dette via metadata i Key Vault.
